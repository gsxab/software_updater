...
func TestBase64URLEncode_Do(t *testing.T) {
	type fields struct {
		Val string
	}
	type args struct {
	...
	tests := []struct {
	...
	}{
		{
			...
			args: args{
				ctx:     nil,
				driver:  nil,
				input:   &action.Args{Strings: []string{"123", "456"}},
				version: nil,
				wg:      nil,
			},
			wantOutput: &action.Args{Strings: []string{"123", "NDU2"}},
			wantExit:   0,
			wantErr:    false,
		},
		{
			...
			wantOutput: &action.Args{Strings: []string{"MTIz", "NDU2"}},
			wantExit:   0,
			wantErr:    false,
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			a, _ := (&Base64URLEncode{}).NewAction(tt.fields.Val)
